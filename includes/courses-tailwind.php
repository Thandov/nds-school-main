...
function nds_courses_page_tailwind()
{
    ...
    if (empty($paginated_courses)) {
        ...
        if (!empty($program_ids)) {
            ...
            $programs_results = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, name FROM {$table_faculties} WHERE id IN ({$program_ids_placeholder})",
                    $program_ids
                ),
                ARRAY_A
            );
    ...
    // ============================================
    $all_query = "SELECT c.*, c.duration_weeks as duration, f.name as program_name
                  FROM {$table_courses} c
                  LEFT JOIN {$table_faculties} f ON c.program_id = f.id";
    ...
    if ($filter_program_id) {
        $current_program = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$table_faculties} WHERE id = %d", $filter_program_id), ARRAY_A);
    }

    // Get programs for dropdown (cached)
    $programs = wp_cache_get('nds_programs_list', 'nds');
    if (false === $programs) {
        $programs = $wpdb->get_results("SELECT id, name as program_name FROM {$table_faculties} ORDER BY name LIMIT 100", ARRAY_A);
    ...
    $recent_courses = $wpdb->get_results("
        SELECT c.id, c.name, c.created_at, f.name as program_name
        FROM {$table_courses} c
        LEFT JOIN {$table_faculties} f ON c.program_id = f.id
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" id="duration" name="duration_weeks"
                                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
