...
function nds_delete_education_path($path_id)
{
    ...
    if ($path_id > 0) {
        ...
        if ($path) {
            // Path exists, proceed to delete
            // First, delete the WordPress page associated with this path
            $page = get_page_by_title($path->name, OBJECT, 'page');

            if ($page) {
                wp_delete_post($page->ID, true);  // true will forcefully delete the page
            }

            // Now, delete the path from the database
            $deleted = $wpdb->delete($table_name, ['id' => $path_id], ['%d']);

            if ($deleted === false) {
                wp_die('Failed to delete faculty: ' . esc_html($wpdb->last_error));
            }

            // After deleting, redirect back to the Faculties list page
            wp_redirect(admin_url('admin.php?page=nds-faculties&deleted=1'));

            exit;  // Make sure exit is here to stop any further code execution
        } else {
            wp_die('The faculty does not exist.');
        }
    } else {
        wp_die('Invalid path ID.');
    }
