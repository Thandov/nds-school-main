...
function nds_add_staff()
{
    ...
    global $wpdb;
    $staff_table = $wpdb->prefix . "nds_staff";
    $_POST['email'] = isset($_POST['first_name']) ? sanitize_text_field($_POST['first_name']) . '@ndsacademy.co.za' : '';
    ...
    $staffMember = $wpdb->insert($staff_table, $data, ['%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s']);
    ...
    // Log create action
    nds_log_staff_action($wpdb->insert_id, get_current_user_id(), 'create_staff', 'create', null, wp_json_encode($data));
...
function nds_handle_update_staff()
{
    ...
    $updated = $wpdb->update($staff_table, $data, ['id' => $staff_id], ['%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s'], ['%d']);
    if ($updated === false) {
