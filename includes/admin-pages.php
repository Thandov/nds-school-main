...
?>
    <div class="wrap">
        ...
        <form method="POST" action="<?php echo admin_url('admin-post.php'); ?>">
            ...
            $typ = "add";
            course_form($typ, null, null, null);
            ?>
        </form>
    </div>
    <?php
...
function nds_create_course_ajax_handler() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'nds_course_nonce')) {
        wp_send_json_error('Security check failed');
    }
    ...
    if (empty($program_id) || empty($course_name)) {
        wp_send_json_error('Please fill in all required fields (program and course name)');
    }
    ...
    if ($result === false) {
        ...
        if ($result === false) {
            wp_send_json_error('Database error: ' . ($wpdb->last_error ?: 'Unknown error'));
        }
    }
