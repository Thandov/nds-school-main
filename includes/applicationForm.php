  ...
  <form id="nds-step1-form">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="password" id="nds_reg_password" name="password" required>
      </div>
      <div>
        <label for="nds_reg_password_confirm">Confirm password</label>
        <input type="password" id="nds_reg_password_confirm" name="password_confirm" required>
      </div>
    </div>
...
<div id="nds-step-2" class="nds-step">
  ...
  <div class="nds-form-step nds-form-step-active" data-step="2">
    ...
    <div class="nds-step-nav">
      <span></span>
      <button type="button" class="nds-button-primary" data-next-step="3">Next: Choose programme</button>
    </div>
  </div>

  <!-- STEP 3: Choose programme -->
  ...
  if (step1Form) {
    step1Form.addEventListener('submit', function(e) {
      ...
      if (!password || password.length < 6 || password !== passwordConfirm) {
        step1Error.textContent = 'Passwords must match and be at least 6 characters.';
        step1Error.style.display = 'block';
        return;
      }
      ...
      if (this.selectedIndex >= 0 && this.options[this.selectedIndex]) {
        ...
        if (courseLevelHidden) {
          ...
          if (fid && pid && ndsCatalogue && ndsCatalogue[fid] && ndsCatalogue[fid].programmes && ndsCatalogue[fid].programmes[pid]) {
            var courses = ndsCatalogue[fid].programmes[pid].courses || {};
