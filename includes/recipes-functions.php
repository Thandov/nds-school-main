...
add_action('admin_post_recipes_delete_recipe', function() {
    ...
    if ($recipe_id > 0) {
        recipes_delete_recipe($recipe_id);
        wp_redirect(admin_url('admin.php?page=nds-recipes&success=deleted'));
        exit;
    }

    wp_redirect(admin_url('admin.php?page=nds-recipes&error=invalid_id'));
    exit;
});
