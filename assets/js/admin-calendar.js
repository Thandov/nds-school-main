...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    console.log('Calendar ID:', calendarId, 'isAdmin:', isAdmin, 'isStaff:', isStaff);
    ...
    var initialView = isAdmin ? 'dayGridMonth' : (isStaff ? 'timeGridWeek' : 'dayGridMonth');

    // Get AJAX config - prioritize based on environment to avoid collisions
    var ajaxConfig = null;
    if (isAdmin) {
      ajaxConfig = window.ndsCalendar;
    } else if (isStaff) {
      ajaxConfig = window.ndsStaffCalendar;
    } else {
      ajaxConfig = window.ndsFrontendCalendar || window.ndsCalendar;
    }
    ...
    var calendarConfig = {
      ...
      viewDidMount: function (info) {
        console.log('View mounted:', info.view.type, info);
        // Handle year view specific layout if needed
      },
      views: {
        timeGridDay: {
        ...
        multiMonthYear: {
          ...
          multiMonthMaxColumns: 4 // Standard FC6 way to show months in a grid
        }
      },
      height: 650,
    ...
    calendar.on('viewDidMount', function (info) {
      // Custom behavior on view mounts
    });
  });
})(jQuery);
