  ...
  $(document).ready(function () {
    ...
    var calendarConfig = {
      ...
      eventClick: function (info) {
        ...
        // Module info
        if (type === 'schedule' && (props.module_name || props.module_code)) {
          content += '<div>';
          content += '<label class="block text-sm font-medium text-gray-700 mb-1">Module</label>';
          var modInfo = [];
          if (props.module_code) modInfo.push('<span class="font-semibold">' + props.module_code + '</span>');
          if (props.module_name) modInfo.push(props.module_name);
          content += '<p class="text-gray-800"><i class="fas fa-book mr-2 text-gray-500"></i>' + modInfo.join(' - ') + '</p>';
          content += '</div>';
        }
